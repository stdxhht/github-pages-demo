...
| 运行开发版 | `python main.py` |
| 打包为单文件 EXE | 执行 `build.bat`，输出 `dist/MyApp.exe` |
| 快速启动（含安装依赖） | 双击 `run.bat` |

> Python 包依赖：PyQt6 + PyQt6-WebEngine。HTML/CSS/JS 为纯前端，无需构建。

## 部署

GitHub Pages 已配置为从 `master` 根目录部署，推送后等待 1-2 分钟即可访问：
```bash
git add .
git commit -m "描述"
git push
```
部署地址：`https://stdxhht.github.io/github-pages-demo/`

## 项目架构

这是一个**混合架构的待办事项应用**：前端使用纯 HTML/CSS/JS（无框架），后端使用 PyQt6 + QWebEngineView 包装为桌面窗口。
...
- 使用 `QWebEngineView` 加载本地 `index.html`（同目录下）。
- 提供顶部工具栏（重新加载按钮 + F5 快捷键），方便开发时刷新页面。
- 通过 `PyInstaller` 可打包为独立 EXE。
